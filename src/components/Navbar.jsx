...
const Navbar = () => {
  ...
  const [transColor, setTransColor] = useState("transparent");

  useEffect(() => {
    window.addEventListener("scroll", () => {
      if (window.scrollY > 0) {
        setTransColor("primary");
      }else{
        setTransColor("transparent");
      }
    });
  },[]);
  ...
  return (
   ...
   <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-${transColor}`}>
